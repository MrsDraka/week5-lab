...
public class Week05OOPLab {
	...
	public static void main(String[] args) {
		...
		Card ace = new Card(cardA, cardS, cardVal);
		...
		Map<String, List<Card>> newGame = createGameBoard(5);
		
		 // Iterating the Map
	    for(Entry<String, List<Card>> entry : newGame.entrySet()){
	        // Use entry.getKey() to fetch Key from entry object 
	        System.out.println("Iterating cards of a player: "+ entry.getKey());
	...
	public static Map<String, List<Card>> createGameBoard(int numPlayers) {
	    ...
	    for (int i = 0; i < 52; i++) {
	        String playerName = "Player " + ((i % numPlayers) + 1);
	        Card drawnCard = deck.draw();
	        gameBoard.get(playerName).add(drawnCard);
	    }
